...
protocol RestaurantRepository {
    func getRestaurants(query: String?) async throws -> [Restaurant]
...
struct GetRestaurantsUseCase {
    ...
    func execute() async throws -> [Restaurant] {
        try await repository.getRestaurants(query: nil)
    }
    
    func executeSearch(query: String) async throws -> [Restaurant] {
        try await repository.getRestaurants(query: query)
    }
}
...
final class RestaurantRepositoryImpl: RestaurantRepository {
    ...
    func getRestaurants(query: String?) async throws -> [Restaurant] {
        var url = baseURL.appending(queryItems: [
